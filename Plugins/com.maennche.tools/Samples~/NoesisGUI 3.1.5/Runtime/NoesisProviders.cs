...
public class NoesisTextureProvider: TextureProvider
{
    ...
    public override void GetTextureInfo(Uri uri, out uint width_, out uint height_)
    {
        ...
        if (_textures.TryGetValue(path, out v))
        {
            if (v.texture != null)
            {
        ...
        // Send to C++
        Noesis_TextureProviderStoreTextureInfo(swigCPtr.Handle, path, width, height, numLevels, nativePtr);
    ...
    public void UpdateTextures()
    {
        ...
        foreach (var kv in textures)
        {
            ...
            RaiseTextureChanged(new Uri(uri, UriKind.Relative));
        }
    }
#endif
